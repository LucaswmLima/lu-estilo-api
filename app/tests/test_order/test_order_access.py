...
class TestNoToken:
    ...
    def test_create_order_unauthorized(self, client: TestClient, create_test_client, create_test_product):
        payload = {
    ...
    def test_update_order_unauthorized(self, client: TestClient, create_test_order, create_test_product):
        payload = {
            "status": "confirmed",
            "products": [{"id": create_test_order.products[0].id, "product_id": create_test_product.id, "quantity": 2}],
        }
...
class TestUserToken:
    ...
    def test_create_order_user_allowed(self, client: TestClient, create_test_client, create_test_product):
        payload = {
            "client_id": create_test_client.id,
            "status": "pending",
            "products": [{"product_id": create_test_product.id, "quantity": 1}],
    ...
    def test_update_order_user_allowed(self, client: TestClient, create_test_order, create_test_product):
        payload = {
            "status": "confirmed",
            "products": [{"id": create_test_order.products[0].id, "product_id": create_test_product.id, "quantity": 3}],
        }
        response = client.put(f"/orders/{create_test_order.id}", json=payload, headers=self.headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_delete_order_user_allowed(self, client: TestClient, create_test_order):
        response = client.delete(f"/orders/{create_test_order.id}", headers=self.headers)
        assert response.status_code == 200

        # Confirm delete
        get_response = client.get(f"/orders/{create_test_order.id}", headers=self.headers)
...
class TestAdminTokenAccess:
    ...
    def test_create_order_admin_allowed(self, client: TestClient, create_test_client, create_test_product):
        payload = {
            "client_id": create_test_client.id,
            "status": "pending",
            "products": [{"product_id": create_test_product.id, "quantity": 1}],
    ...
    def test_update_order_admin_allowed(self, client: TestClient, create_test_order, create_test_product):
        payload = {
            "status": "shipped",
            "products": [{"id": create_test_order.products[0].id, "product_id": create_test_product.id, "quantity": 5}],
        }
        response = client.put(f"/orders/{create_test_order.id}", json=payload, headers=self.headers)
        assert response.status_code == 200
        data = response.json()
