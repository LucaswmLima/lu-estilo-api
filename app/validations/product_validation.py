...
def validate_unique_barcode(db: Session, barcode: Optional[str], product_id: int = None):
    if not barcode:
        return
...
def validate_expiration_date(expiration_date: Optional[date]):
    ...
    if expiration_date < date.today():
        raise HTTPException(status_code=400, detail="Validity data cannot be in the past")
